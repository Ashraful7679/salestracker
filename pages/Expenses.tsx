...
import { Wallet, ArrowDownCircle, ArrowUpCircle, History, Plus, User, DollarSign } from 'lucide-react';

type Tab = 'expense' | 'withdrawal' | 'salary';

export default function Expenses() {
  const { user } = useAuth();
  const isAdmin = user?.role === 'admin';
  const { cashFlows, addCashFlow, employees, paySalary } = useStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (activeTab === 'salary') {
      ...
      await paySalary(selectedEmployeeId, parseFloat(amount), description);
    } else {
      await addCashFlow({
        type: activeTab,
  ...
  const filteredFlows = cashFlows.filter(c => {
    if (activeTab === 'salary') return c.category === 'Salary';
    return c.type === activeTab && c.category !== 'Salary';
  });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden h-fit">
          <div className="p-1 bg-slate-100 m-4 rounded-lg flex gap-1">
            ...
            <button
              ...
              className={`flex-1 py-2 text-sm font-medium rounded-md flex items-center justify-center gap-2 transition-all ${activeTab === 'salary' ? 'bg-white text-green-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            {activeTab === 'salary' && (
              <div>
                ...
                >
                  ...
                  {employees.map(emp => (
                    <option key={emp.id} value={emp.id}>{emp.name} (Due: ${emp.totalDueSalary.toFixed(2)})</option>
                  ))}
                </select>
              </div>
            )}
            ...
            <button
              type="submit"
              className={`w-full py-3 rounded-lg text-white font-bold flex items-center justify-center gap-2 transition-colors ${activeTab === 'expense' ? 'bg-red-600 hover:bg-red-700' :
                activeTab === 'withdrawal' ? 'bg-amber-500 hover:bg-amber-600' :
                  'bg-green-600 hover:bg-green-700'
                }`}
            >
              <Plus className="w-5 h-5" />
              Record {activeTab === 'expense' ? 'Expense' : activeTab === 'withdrawal' ? 'Withdrawal' : 'Salary Payment'}
            </button>
          </form>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col">
          <div className="p-4 border-b border-slate-200 flex items-center gap-2">
            ...
            <h3 className="font-bold text-slate-800">Recent {activeTab === 'expense' ? 'Expenses' : activeTab === 'withdrawal' ? 'Withdrawals' : 'Salary Payments'}</h3>
          </div>
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50 text-slate-500 sticky top-0">
                  <tr>
                    ...
                    {(activeTab === 'expense' || activeTab === 'salary') && <th className="px-6 py-3 font-semibold">Category</th>}
                    <th className="px-6 py-3 font-semibold">Description</th>
                    {isAdmin && <th className="px-6 py-3 font-semibold">Author</th>}
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {filteredFlows.map(item => (
                    <tr key={item.id} className="hover:bg-slate-50">
                      ...
                      {(activeTab === 'expense' || activeTab === 'salary') && (
